/*
	...
	LocalizaTipoDado: essa função retorna uma lista dos tipos pokemon presentes no json conforme dado solicitado.
	Exemplo LocalizaTipoDado('bug', 'doDoubleDamage') trará todos os tipos pokemon que causam o dobro de dano ao tipo 'bug';
	
	verificaQualTipoUsarContraTipo: parecida com a função LocalizaTipoDado, porém retorna apenas o tipo com a maior quantidade de
	vantagens. Exemplo erificaQualTipoUsarContraTipo('bug', 'doDoubleDamage') trará o tipo 'fire', pois além de dar o dobro de dano
	no tipo 'bug' também da o dobro de dano em 'grass', 'ice' e 'steel', sendo o mais efetivo.
	
	montaTimeContraTipos: parecida com a função verificaQualTipoUsarContraTipo, porém recebe um array com os tipos como argumento com
	o objetivo de montar um time contra todos os tipos informados. Exemplo montaTimeContraTipos(['bug', 'fire'], 'doDoubleDamage').
	
	por enquanto é só... não possui interface gráfica pois é apenas um projeto de lógica de programação. Não possui um bd com os pokemons
	de cada tipo para auxiliar na escolha para compor o time.
